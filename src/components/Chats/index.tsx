...
const Chats: React.FC = () => {
  ...
  const [selectedChat, setSelectedChat] = useState<EnhancedChatInfo | null>(
    null
  );
    ...
    const handleUserChats = async (userChats: IUserChatInfo[]) => {
      try {
        ...
        const validChats = userChats.filter(
          (chat) => chat && chat.otherUserId && chat.matchId
        );
        ...
        const enhancedChats = await Promise.all(
          validChats.map(async (chat) => {
            try {
              ...
              const userItem =
                item1.userId === currentUser?._id ? item1 : item2;
              const otherItem =
                item1.userId === currentUser?._id ? item2 : item1;
              ...
              console.error(
                `Error fetching details for chat ${chat.matchId}:`,
                error
              );
              return {
                ...chat,
    ...
    const handleNewMessage = (message: IChatMessage) => {
      ...
      setChats((prevChats) => {
        const chatIndex = prevChats.findIndex(
          (chat) => chat.matchId === message.matchId
        );
        if (chatIndex === -1) {
          ...
          <div className="chat-header-info">
            ...
            <span
              className={`chat-status ${
                onlineUsers.has(selectedChat.otherUserId) ? "online" : "offline"
              }`}>
